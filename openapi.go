...
// OpenAPIFormatter implements a markdown doc generator. It is
...
func (oas *OpenAPIFormatter) GenerateIntro() string {
...
info:
  title: HTTP API
  description: |
    HTTP API reference fosr IPFS, the InterPlanetary File System.
...
func (oas *OpenAPIFormatter) GenerateIndex(endps []*Endpoint) string {
	buf := new(bytes.Buffer)
...
func (oas *OpenAPIFormatter) GenerateEndpointBlock(endp *Endpoint) string {
	buf := new(bytes.Buffer)
	fmt.Fprintf(buf, `
    %s:
      post:
...
func (oas *OpenAPIFormatter) GenerateArgumentsBlock(args []*Argument, opts []*Argument) string {
	buf := new(bytes.Buffer)
	...
	for _, arg := range args {
		fmt.Fprintf(buf, genParameter(arg, true))
	}

	for _, opt := range opts {
		fmt.Fprintf(buf, genParameter(opt, false))
	}

	return buf.String()
}

func (oas *OpenAPIFormatter) GenerateBodyBlock(args []*Argument) string {
...
func (oas *OpenAPIFormatter) GenerateResponseBlock(response string) string {
	buf := new(bytes.Buffer)
	fmt.Fprintf(buf, `
        responses:
          200:
            description: On success, the call to this endpoint will return with 200 and the following body
            content: {}
...
func (oas *OpenAPIFormatter) GenerateExampleBlock(endp *Endpoint) string {
	buf := new(bytes.Buffer)
